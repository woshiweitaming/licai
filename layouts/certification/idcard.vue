	...
	<view class="upload_class">
		<view class="images_list">
			...
			<view class="form_wrap">
				<view class="btns" @click="uploadIdcardHandler">{{getLang('certification_p11')}}</view>
			</view>
		</view>
	</view>
</template>
...
<script>
	...
	import { uploadIdImg } from '@/apis/users.js'
	export default {
		name: 'Idcard',
		mixins: [langMixins],
		data(){
		...
		methods: {
			...
			async uploadIdcardHandler(){	
				if(this.form.p1.length === 0){
					return tips.showToast(this.getLang('fund_p29'))
				}
				if(this.form.p2.length === 0){
					return tips.showToast(this.getLang('fund_p30'))
				}
				if(this.form.p3.length === 0){
					return tips.showToast(this.getLang('fund_p31'))
				}
				const res = await uploadIdImg()
