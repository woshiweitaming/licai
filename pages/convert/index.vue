<template>
	<view class="delivery_address">
		<view class="da_main">
			<view class="exchange">
				<view class="name">{{getLang('buy_p1')}}({{getLang('buy_p6') + '：' + (urlParams.jifen || 0)}})</view>
				<view class="input_bars">
					<input type="text" v-model="num" :placeholder="getLang('buy_p2')" class="input_bar" />
				</view>
			</view>
			<view class="address">
		...
		<view class="bottom_bar">
			<view class="btns" @click="exchangeCommodityHandler">{{getLang('buy_p5')}}</view>
		</view>
	</view>
</template>
...
<script>
	...
	import { getAdds, exchangeCommodity } from '@/apis/users.js'
	...
	export default {
		...
		methods: {
			...
			async exchangeCommodityHandler(){
				if(this.num.length === 0){
					return tips.showToast(this.getLang('buy_p2'))
				}
				if(Number(this.num) > Number(this.urlParams.jifen)){
					return tips.showToast(this.getLang('buy_p7'))
				}
				const params = {
					sid: this.urlParams.id,
					num: this.num,
					aid: this.dataList[this.cur].id
				}
				await exchangeCommodity(params)
			}
		},
		onShow() {
			this.getAddsHandler()
