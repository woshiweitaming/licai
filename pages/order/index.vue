<template>
	<view class="piggy_bank">
		...
		<view class="balance">
			...
			<view class="block">
				<view class="labels">
					<view class="value">{{details.min_amount}}</view>
					<view class="name">{{getLang('order_p4')}}{{getLang('order_p12')}}</view>
				</view>
				<view class="labels">
					<view class="value">{{(Number(details.min_amount || 0) * (Number(opt.deal_rate || 0) / 100)).toFixed(2)}}</view>
			...
			<view class="info info2">
				<view class="labels">
					...
					<view class="value value2">
						<u-number-box input-width="180" :step="100" :min="Number(opt.min_amount) || 0" v-model="amount" @change="valChange"></u-number-box>
					</view>
				</view>
				<view class="labels">
...
<script>
	...
	export default {
		...
		methods: {
			valChange(val){
				if(val.value < Number(this.opt.min_amount)){
					console.log('错误')
				}
			},
		...
		onLoad(opt) {
			this.opt = opt
			this.amount = Number(opt.min_amount)
