<template>
	<view class="product_wrap">
		...
		<u-gap height="20" bg-color="#f5f5f3"></u-gap>
		<Question :pname="pro_name" :period="summary.period" />
...
<script>
	...
	export default {
		...
		onLoad(opt) {
			this.opt = opt
			const { min_amount, period, deal_rate, total_amount, progress, flg, djs } = opt
			this.summary = { min_amount, period, deal_rate }
			this.tagList = opt.label.split(',')
			this.pro_name = opt.pro_name
			this.details = { min_amount, progress, period, deal_rate, flg, djs }
